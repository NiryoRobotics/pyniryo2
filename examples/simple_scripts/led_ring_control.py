...
print 'SOLID'
niryo_robot.led_ring.led_ring_solid([255,0,0])
...
niryo_robot.led_ring.led_ring_alternate([[0,255,0], [255,0,0], [0,0,255]], iterations = 8)

time.sleep(4)

print 'CHASE with 100ms between each step '
niryo_robot.led_ring.led_ring_chase([178,78,100], speed = 100)

time.sleep(4)

print 'WIPE 3 times with 90ms between each step, wait for the end'
niryo_robot.led_ring.led_ring_wipe([98,78,190], speed = 90, wait = True)

print 'RAINBOW 2 times with 10ms between each step '
niryo_robot.led_ring.led_ring_rainbow(speed = 10, iterations = 2)

time.sleep(4)

print 'RAINBOW CYCLE endless'
niryo_robot.led_ring.led_ring_rainbow_cycle()

time.sleep(4)

print 'RAINBOW CHASE endless'
niryo_robot.led_ring.led_ring_rainbow_chase()

time.sleep(4)

print 'GO UP 4 times with 70ms between each step, wait for the end'
niryo_robot.led_ring.led_ring_go_up([189,96,113], iterations = 4, speed = 70, wait = True)

print 'GO UP AND DOWN 5 times, wait for the end'
niryo_robot.led_ring.led_ring_go_up_down([70,167,97], iterations = 5, wait = True)
