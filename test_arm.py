...
class TestHardwareStatus(BaseTest):
    ...
    def test_shift(self):
        ...
        self.assertAlmostEqualVector(self.arm.get_pose().to_list(), [0.25, 0., 0.25, 0, 1., 0])

        self.assertIsNone(self.arm.shift_pose(RobotAxis.PITCH, -1.))
        self.assertAlmostEqualVector(self.arm.get_pose().to_list(), [0.25, 0., 0.25, 0, 0., 0.])

        self.assertIsNone(self.arm.shift_pose(RobotAxis.PITCH, 1., end_move_callback))
        self.assertTrue(end_move_event.wait(10))
        self.assertAlmostEqualVector(self.arm.get_pose().to_list(), [0.25, 0., 0.25, 0, 1., 0.])

        self.assertIsNone(self.arm.shift_pose(RobotAxis.ROLL, 0.5))
        self.assertAlmostEqualVector(self.arm.get_pose().to_list(), [0.15, 0., 0.3, 0.5, 1., 0.])

        self.assertIsNone(self.arm.shift_pose(RobotAxis.YAW, .5))
        self.assertAlmostEqualVector(self.arm.get_pose().to_list(), [0.25, 0., 0.25, 0.5, 1., .5])
    ...
    def test_jog_pose(self):
        ...
        self.assertIsNone(self.arm.move_pose([0.29, 0.0, 0.3, 0.0, 0.0, 0.0]))
        self.assertAlmostEqualVector(self.arm.get_pose().to_list(), [0.29, 0.0, 0.3, 0.0, 0.0, 0.0])
    ...
    def test_kinematics(self):
        ...
        # Inverse Kinematics
        pose_target = [0.17, 0.0, 0.3, 0.0, 0.0, 0.0]
